...
//##### BASE BUILDING 1.2 ADDED CODE1 #### START1 - Daimyo
	...
	//Build Recipe Menu Action
		...
		if (s_player_recipeMenu < 0) then {
			s_player_recipeMenu = player addaction [("<t color=""#0074E8"">" + ("Build Recipes") +"</t>"),"actions\player_recipeBook.sqf","",5,false,true,"",""];
		};
	} else {
...
camoNet_Nato distance player < 10)) then {
		if (s_player_deleteCamoNet < 0) then {
			s_player_deleteCamoNet = player addaction [("<t color=""#F01313"">" + ("Remove Netting") +"</t>"),"actions\player_remove.sqf","",1,true,true,"",""];
		};
	} else {
		player removeAction s_player_deleteCamoNet;
		s_player_deleteCamoNet = -1;
	};
// ##### BASE BUILDING 1.2 ADDED CODE1 #### END1 - Daimyo
...
if (!isNull cursorTarget and !_inVehicle and (player distance cursorTarget < 4)) then {	//Has some kind of target
	...
	//##### BASE BUILDING 1.2 ADDED CODE2 #### START2 - Daimyo
	
	// Enter Code to Operate Gates Action
	if((speed player <= 1) && !keyValid && cursortarget iskindof "Infostand_2_EP1" && cursorTarget distance player < 5 && _canDo) then {
		if (s_player_enterCode < 0) then {
			s_player_enterCode = player addaction [("<t color=""#4DFF0D"">" + ("Enter Key Code to Operate Gate") +"</t>"),"keypad\fnc_keyPad\enterCode.sqf","",5,false,true,"",""];
		};
	} else {
	...
	// Operate Gates by KillZode Kid and Humbleuk
	if ((dayz_myCursorTarget != cursorTarget) && (cursorTarget isKindOf "Infostand_2_EP1") && keyValid) then {
		...
		_gates = nearestObjects [_lever, ["Concrete_Wall_EP1"], 100]; // Action will only show when these objects are around 100 meters
		if (count _gates > 0) then {
			_handle = dayz_myCursorTarget addAction ["Operate Gate", "keypad\fnc_keyPad\operate_gates.sqf", _lever, 1, false, true, "", ""];
			s_player_gateActions set [count s_player_gateActions,_handle];
		};
	};
	// Remove Buildable for Base Building 1.2
	if((typeOf(cursortarget) in allremovables) && _hasToolbox && _canDo && !remProc && !procBuild) then {
	...
	// Disarm Booby Trap Action for Base Building 1.2
	if(_hasToolbox && _canDo && !remProc && !procBuild && (cursortarget iskindof "Grave" && cursortarget distance player < 2.5 && !(cursortarget iskindof "Body" || cursortarget iskindof "GraveCross1" || cursortarget iskindof "GraveCross2" || cursortarget iskindof "GraveCrossHelmet" || cursortarget iskindof "Land_Church_tomb_1" || cursortarget iskindof "Land_Church_tomb_2" || cursortarget iskindof "Land_Church_tomb_3" || cursortarget iskindof "Mass_grave"))) then {
		if (s_player_disarmBomb < 0) then {
			s_player_disarmBomb = player addaction [("<t color=""#F01313"">" + ("Disarm Bomb") +"</t>"),"actions\player_disarmBomb.sqf","",1,true,true,"", ""];
		};
	} else {
		player removeAction s_player_disarmBomb;
		s_player_disarmBomb = -1;
	};
	//##### BASE BUILDING 1.2 ADDED CODE2 #### END2 - Daimyo
	...
	*/
	...
	//#### Base Building 1.2 Remove Actions ### Start3
	{dayz_myCursorTarget removeAction _x} forEach s_player_gateActions;s_player_gateActions = [];
	dayz_myCursorTarget = objNull;
	player removeAction s_player_deleteBuild;
	s_player_deleteBuild = -1;
	...
	player removeAction s_player_enterCode;
	s_player_enterCode = -1;
	//#### Base Building 1.2 Remove Actions ### End3
