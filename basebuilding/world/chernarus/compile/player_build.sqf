...
_closestTown = (nearestLocations [player,["NameCityCapital","NameCity","NameVillage"],5000]) select 0;
...
closedialog 1;
// Ashfor Fix: Did player try to drop mag and keep action active (not really needed but why not)
_item = _this;
...
switch (true) do
{
	case(_longWait):
	{
		...
		for "_i" from 0 to _longWloop do
		{
			...
			if (player distance _locationPlayer > 1) then {cutText [format["Build canceled for %1, position of player moved",_text], "PLAIN DOWN",1]; procBuild = false;_playerCombat setVariable["startcombattimer", 1, true]; breakOut "exit";};
			if (!_canDo || _onLadder || _inVehicle || _isWater) then {cutText [format["Build canceled for %1, player is unable to continue",_text], "PLAIN DOWN",1]; procBuild = false;_playerCombat setVariable["startcombattimer", 1, true]; breakOut "exit";};
